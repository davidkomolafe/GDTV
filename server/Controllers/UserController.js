...
const loginUser = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // if user exists then compare password with hashed password then send user data and token to client
...
const deleteUserProfile = asyncHandler(async (req, res) => {
  try {
    ...
    if (user) {
      ...
      await User.remove();
      res.join({ message: "User deleted successfully" });
    }
    //else send error message
    else {
      res.status(404);
...
const changeUserPassword = asyncHandler(async (req, res) => {
  ...
  try {
    //find user in DB
    const user = await User.findById(req.user._id);
    //if user exists compare old password with hashed password and then update user password and save it in db
...
const getLikedMovies = asyncHandler(async (req, res) => {
  try {
    ...
    // if user exists send linked movies to client
    if (user) {
